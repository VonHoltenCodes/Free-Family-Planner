...
try {
    ...
    if ($op === 'call') { $ent = $body['entity'] ?? ''; $dom = explode('.', $ent)[0]; $act = $body['action'] ?? '';
        if ($act === 'snapshot') { if ($dom !== 'camera') $out(400, ['error' => 'snapshot is for cameras']); hub_req('POST', "$url/api/services/blink/trigger_camera", $tok, ['entity_id' => $ent]); $out(200, ['ok' => true]); }
