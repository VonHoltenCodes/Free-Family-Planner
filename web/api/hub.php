...
try {
    ...
    if ($op === 'entities' || $op === 'states') {
        ...
        foreach ($states as $s) { $eid = $s['entity_id'] ?? ''; $dom = explode('.', $eid)[0]; $a = $s['attributes'] ?? [];
            ...
            elseif (in_array($eid, $ids, true)) { $keep = []; foreach (['temperature', 'current_temperature', 'hvac_action', 'target_temp_high', 'target_temp_low', 'brightness', 'battery_level', 'media_title', 'media_artist', 'source', 'percentage', 'thumbnail', 'motion_enabled', 'motion_detected', 'battery', 'last_record', 'brand'] as $k) if (isset($a[$k])) $keep[$k] = $a[$k];
    ...
    if ($op === 'call') { $ent = $body['entity'] ?? ''; $dom = explode('.', $ent)[0]; $act = $body['action'] ?? '';
        if ($act === 'snapshot') { if ($dom !== 'camera') $out(400, ['error' => 'snapshot is for cameras']); hub_req('POST', "$url/api/services/blink/trigger_camera", $tok, ['entity_id' => $ent]);
            // PHP has no background thread: finish the response, then poke HA to re-read the camera so the new thumbnail lands quickly
            ignore_user_abort(true); echo json_encode(['ok' => true]); if (function_exists('fastcgi_finish_request')) fastcgi_finish_request(); else { header('Connection: close'); header('Content-Length: ' . ob_get_length()); ob_end_flush(); flush(); }
            foreach ([6, 12, 20] as $d) { sleep($d); try { hub_req('POST', "$url/api/services/homeassistant/update_entity", $tok, ['entity_id' => $ent]); } catch (Exception $e) {} } exit; }
