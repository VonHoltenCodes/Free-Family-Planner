...
export function openSetup(current, { firstRun = false, step: startStep = 0 } = {}) {
  ...
  const render = () => {
    ...
    if (step === 0) {
      ...
      drawTiles();
      const search = text('', 'filter entities…', () => drawEnts()); const entList = el('div', 'ent-list');
      const refreshBtn = el('button', 'btn sm', '↻ Refresh from hub'); refreshBtn.type = 'button';
      refreshBtn.addEventListener('click', async () => { refreshBtn.disabled = true; try { entities = await hub.entities(); drawEnts(); note(`${entities.length} entities from the hub`, 'ok'); } catch (e) { note(`refresh failed: ${e.message}`, 'bad'); } refreshBtn.disabled = false; });
      const addRow = el('div', 'row-inline'); addRow.append(search, refreshBtn); hubBox.append(field('Add a tile', addRow, 'the list is fetched live each time this step opens; use Refresh after adding a device in Home Assistant'), entList);
