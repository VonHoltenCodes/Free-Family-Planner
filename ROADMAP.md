...
| **Home hub interop** | `feat/home-hub` | Generic hub adapter, Home Assistant first, Home-IO second: the planner exposed as sensors (`/api/state`), two-way shopping-list sync with the hub's to-do list, and a read-only House panel (thermostat, doors, who's home). Hub token stays server-side. Issue #19. |
