...
Two ways, in ⚙ Setup → Calendars:
- **On the server (recommended for a wall display).** Paste an OAuth client ID *and secret*, press
  Connect, approve once at Google. The server keeps a refresh token and mints access tokens; the
  display never signs in, survives reloads, and works from any address.
- **In this browser.** Client ID only, a *Sign in with Google* button on the display. Simpler, but
  somebody has to tap it again when the token lapses, and Google only accepts `http://localhost` or
  `https://` origins.

Full walkthrough, including the exact console steps and the *Testing vs Published* seven-day refresh
token trap: [docs/google-calendar.md](docs/google-calendar.md).
